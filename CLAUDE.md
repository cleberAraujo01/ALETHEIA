...
- **Toda mudança no `diff-engine` reporta os pares da bancada** (`pnpm corpora:medir`) — juventude, oscar e, desde 2026-08-17, **Sauce Demo** (`__corpus__/saucedemo/`, público, com ações: 4 usuários com defeito + piso; 6 de 11 bloqueados, 11 de 11 visíveis, FP 0%) —, mais os pisos, e **toda regra nova de severidade ou normalização passa por piso de ruído em aplicação de stack estranha** antes de ser considerada estável. Duas capturas da mesma build, dois minutos, já derrubaram regra três vezes.
