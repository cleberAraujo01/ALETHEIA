...
| 4 | **E-02** diff de banco (read-only) — **feita (2026-08-17)** | `packages/capabilities` (spec §14.3, validador estático **léxico, não AST**, executor `READ` com sqlite via `node:sqlite` em modo somente leitura, PII mascarada por hash na borda, auditoria sem valores), sonda `database` no `observe` da IR, camada `DATABASE` no diff (alinhamento por `keyColumns` declaradas, voláteis por declaração), `--base-db/--head-db/--capabilities` na CLI | `pnpm demo:banco`: mesma tela, desconto 15 → 10 só no banco → `DB_FIELD_CHANGED` HIGH, reprova. Suíte de SQL malicioso no CI. **PostgreSQL e `EXPLAIN` entram com o primeiro piloto que os tiver** |
| 5 | **E-07** ambiente efêmero via template clone — **feita (2026-08-17), metade dos dados** | `provisionEphemeralDatabase`: `template-clone` em sqlite (cópia) e **PostgreSQL (`CREATE DATABASE … TEMPLATE`, `DROP` no fim)**, adaptador PostgreSQL (`READ ONLY` + `statement_timeout`), `dataStrategy` nos metadados (PA-12) e no comentário; `--data-strategy` na CLI; Postgres de serviço no CI | PA-06: descartabilidade, não limpeza. **A aplicação por PR continua vindo da plataforma do cliente** (preview URL) — o `confidenceMode` só vira `ISOLATED` quando app e dados forem por execução, e até lá o relatório diz os dois. `pnpm demo:banco --postgres` |

> **As cinco fatias de engenharia estão feitas (2026-08-17).** A fase NÃO está fechada: o critério de saída é comercial — demo em aplicação de cliente, 3 pilotos, NPS. O piloto juventude roda desde 2026-08-22 (§9 da medição). O que resta é uso real; código novo agora só entra puxado por piloto. **Kit de onboarding desde 2026-09-22 (§11 da medição):** `aletheia init --url <produção>` descobre as rotas num browser real e escreve jornada e workflow; validado contra o juventude (as mesmas 7 rotas da mão) e contra o ambiente de validação do Cleber, `cleberAraujo01/aletheia-demo` (loja estática com regras de negócio quebráveis; 6 rotas, piso 0). Piloto novo começa por ele.
...
- **Toda mudança no `diff-engine` reporta os pares da bancada** (`pnpm corpora:medir`) — juventude, oscar e, desde 2026-08-17, **Sauce Demo** (`__corpus__/saucedemo/`, público, com ações: 4 usuários com defeito + piso; 6 de 11 bloqueados, 11 de 11 visíveis, FP 0%) e **Vite docs** (`__corpus__/vite-docs/`, PRs reais do vitejs/vite, preview público × produção: 3 PRs legítimos com **86 bloqueantes, todos falso positivo — a maior parte é do par**, preview construído da base do PR contra a `main` de hoje; e o **#23201, build quebrada de verdade, 1 de 1 bloqueado, FP 0%** — a primeira regressão do projeto que ninguém injetou) e, desde 2026-09-22, **Excalidraw** (`__corpus__/excalidraw/`, PRs reais com **par exato**: a Vercel publica preview por commit da `master`, então a base é o merge-base do PR e não há drift; 5 PRs legítimos, jornada com ações, **4 bloqueantes falso positivo, todos no PR que reestrutura o diálogo em que aparecem**, 0 nos outros quatro, piso 0; ensinou que hash do Vite sem dígito escapava de `NORM-NET-007` de um lado só, e que a dedup da supressão por `deltaId` contava seis execuções reais como uma — os dois corrigidos no mesmo dia, §10.4 da medição: exceção estreita para o hash base64url de 8 caracteres, e evidência com `pairId` (o mesmo par re-diffado não conta; outra captura conta). As 5 regras do Excalidraw têm 6 execuções e **continuam `PROPOSED`** — ativar é revisão humana) —, mais os pisos, e **toda regra nova de severidade ou normalização passa por piso de ruído em aplicação de stack estranha** antes de ser considerada estável. Duas capturas da mesma build, dois minutos, já derrubaram regra cinco vezes — e as cinco foram identidade de sessão ou token (`NORM-NET-010`, o token de clique de anúncio no `href`, é a última), nunca severidade.
- **O achado do `_rsc` foi fechado pelo piloto (2026-08-22, §9.6 da medição da Fase 1):** na rodada 2 do juventude ele bloqueou um PR idêntico com 4 `REQUEST_REMOVED` HIGH. `NORM-NET-013` funde o token de contexto do roteador, o fato `speculative` rebaixa cardinalidade de prefetch a LOW (visível, nunca bloqueante) e corpo `null` do prefetch abortado deixa de ser comparado. 4→0 no par real; bancada idêntica. **Não alargue a classe especulativa além de `_rsc`** sem outro par real que a peça.
